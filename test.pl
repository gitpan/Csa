...
eval
{
    print "Calendars: ", join(", ", Csa::list_calendars($host)),"\n";
};
print ($@) if ($@);
...
$session->short_entry_names(1);
$session->unix_times(1);
#Csa::generate_numeric_enumerations(1);	# 0 is default
#Csa::accept_numeric_enumerations(1);	# 0 is default

#$user = $session->look_up({name => $user});
#print Dumper($user);

print "SUBTYPE_APPOINTMENT = '", Csa::SUBTYPE_APPOINTMENT, "'\n";;
...
eval
{
    print Dumper([Csa::list_calendars($host)]);
};
print ($@) if ($@);
...
print "Retrieve...";
$ret = Csa::lookup($session, 870478200, 881478200);
if ($ret == Csa::CSA_SUCCESS)
{
    print "ok\n";
}
else
